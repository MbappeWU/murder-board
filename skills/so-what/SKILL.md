---
name: so-what
description: Make any update, report, email, memo, deck outline or weekly report exec-ready by asking "so what?" of every line. Finds the one thing the reader must know or decide, rebuilds the text answer-first with the Pyramid Principle (SCQA, MECE support), cuts context and weasel words, and returns three lengths - a one-line subject or chat message, a three-line BLUF summary, and the full rewrite - plus a red-pen report of what was cut and why. Includes Chinese workplace formats (周报, 汇报, 请示). Use when the user asks to make something shorter, clearer, more executive, summarize for their boss or leadership, write a status update or weekly report, or says BLUF, TL;DR, exec summary, so what, 说人话, 结论先行, 写周报, 改汇报, 太长了, 领导看不懂.
license: MIT
metadata:
  author: MbappeWU
  version: "1.0.0"
